...
\node[bnd] (qmc) at (2,1) {single level qMC cubature error bounds};
\node[bnd,fill = lightyellow] (apostbd) at (7,4) {a posteriori error \textbf{bounds} for FEM for PDEs};
\node[bnd,fill = lightorange] (qmcml) at (7,2) {multi-level qMC cubature error bounds};
\node[bnd,fill = lightyellow] (apostrandbd) at (12,3) {a posteriori error \textbf{bounds} for PDEs with randomness};
