...
\node[bnd = lightyellow] (proposed) at (9.5,6) {proposed};
\node[bnd = lightorange] (in-progress) at (7,1) {in progress};
...
\node[bnd = lightorange] (qmcml) at (7,2.5) {a posteriori error \textbf{bounds} for multi-level qMC cubature} edge (qmc);
\node[bnd = lightyellow] (apostbd) at (7,4.5) {a posteriori FEM error \textbf{bounds} for deterministic PDEs} edge (apostest) edge (uni);
\node[bnd = lightyellow] (apostrandbd) at (12,3.5) {a posteriori error \textbf{bounds} for PDEs with \textbf{randomness}} edge (apostbd) edge (qmcml);
\end{tikzpicture}
\end{figure}
