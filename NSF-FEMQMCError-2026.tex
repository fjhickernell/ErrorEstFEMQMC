...
\node[bnd] (apostest) at (2,5) {a posteriori error \textbf{estimates} for FEM for PDEs};
\node[bnd] (uni) at (2,3) {a posteriori univariate function approximation error bounds \cite{ChoEtal17a}};
\node[bnd] (qmc) at (2,1) {a posteriori single level qMC cubature error bounds \cite{HicEtal17a}};
\node[bnd,fill = lightyellow] (apostbd) at (7,4.5) {a posteriori error \textbf{bounds} for FEM for PDEs};
\node[bnd,fill = lightorange] (qmcml) at (7,2.5) {a posteriori multi-level qMC cubature error bounds};
\node[bnd,fill = lightyellow] (apostrandbd) at (12,3.5) {a posteriori error \textbf{bounds} for PDEs with randomness};
