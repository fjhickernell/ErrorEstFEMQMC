...
\node[bnd,fill = lightyellow] (apostbd) at (7,4.5) {a posteriori FEM error \textbf{bounds} for deterministic PDEs};
\node[bnd,fill = lightorange] (qmcml) at (7,2.5) {a posteriori error \textbf{bounds} for multi-level qMC cubature};
\node[bnd,fill = lightyellow] (apostrandbd) at (12,3.5) {a posteriori error \textbf{bounds} for PDEs with \textbf{randomness}};
\draw[->,ultra thick] (apostest.east) -- (apostbd.170);
\draw[->,ultra thick] (uni.east) -- (apostbd.190);
\draw[->,ultra thick] (qmc.east) -- (qmcml.west);
\draw[->,ultra thick] (qmcml.east) -- (apostrandbd.190);
\draw[->,ultra thick] (apostbd.east) -- (apostrandbd.170);
\end{tikzpicture}
\end{figure}
