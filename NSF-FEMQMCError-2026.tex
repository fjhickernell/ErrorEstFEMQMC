...
\begin{center}
\Large \textbf{
Theoretically Sound Adaptive Methods for Elliptic Partial Differential Equations with Randomness
}
\end{center}
%\vspace{-2ex}
%
...
\node[bnd] (exist) at (2,6.5) {existing};
\node[bnd, fill = lightyellow] (proposed) at (9.5,6) {proposed};
\node[bnd, fill = lightorange] (in-progress) at (7,1) {in progress};
\node[bnd] (qmc) at (2,1) {a posteriori error \textbf{bounds} for single level qMC cubature \cite{HicEtal17a}};
\node[bnd] (uni) at (2,3) {a posteriori error \textbf{bounds} for univariate function approximation  \cite{ChoEtal17a}};
\node[bnd] (apostest) at (2,5) {a posteriori FEM error \textbf{estimates} for deterministic PDEs};
\node[bnd,fill = lightorange] (qmcml) at (7,2.5) {a posteriori error \textbf{bounds} for multi-level qMC cubature} edge (qmc);
\node[bnd,fill = lightyellow] (apostbd) at (7,4.5) {a posteriori FEM error \textbf{bounds} for deterministic PDEs} edge (apostest) edge (uni);
\node[bnd,fill = lightyellow] (apostrandbd) at (12,3.5) {a posteriori error \textbf{bounds} for PDEs with \textbf{randomness}} edge (apostbd) edge (qmcml);
\end{tikzpicture}
\end{figure}
...
%	\begin{tikzpicture}
%		\draw[help lines] grid (15,7);
%		\node[bnd] (qmc.east) at (2,1) {a posteriori error \textbf{bounds} for single level qMC cubature \cite{HicEtal17a}};
%		\node[bnd,fill = lightorange] (qmcml.west) at (7,2.5) {a posteriori error \textbf{bounds} for multi-level qMC cubature}
%		edge[<-,ultra thick] (qmc.east);
%	\end{tikzpicture}
%\end{figure}
